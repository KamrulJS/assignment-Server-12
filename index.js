...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.znxhrwq.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const usersCollection = client.db("AudioPair").collection("users");
    const classesCollection = client.db("AudioPair").collection("Classes");
      const instructorsCollection = client.db("AudioPair").collection("Instructors");
    const selectedClassCollection = client.db("AudioPair").collection("selected");
    const paymentCollection = client.db("AudioPair").collection('payments');
    const enrolledClassCollection = client.db('AudioPair').collection('enrolledClass');
